...
//Fix from Guangda incoming for D Minor - SKIP THIS FOR NOW
...
export async function statusContext() {
  //Grab lead status ID
  const list_user_api = 'v1/lead_statuses'
  const prom = await sdk.api(list_user_api, {method : 'GET', body: '{}'})
  const fieldName = 'status_id'
  var fieldValue = ''

  //Valid Status ID
  //iterating over all the status ID
...
export async function companyContext() {
  ...
  //Valid Input
  var fieldValue = 'Company Updated'
  await handleSaveContext(fieldName, fieldValue)
  await handleCompareContext(fieldName, fieldValue)
